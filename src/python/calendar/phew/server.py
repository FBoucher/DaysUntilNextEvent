...
def run(host = "0.0.0.0", port = 80):
    ...
    try:
        os.stat("wifi.json")
        timeout = 60*5 # 5 minutes if there is an existing password
    except:
        timeout = 3600*5 # 5 hours if there is no password
    print("run for ", str(timeout))
    async def timeout_completion():
        await uasyncio.sleep(timeout)
        machine.reset()
    try:
        loop.run_until_complete(timeout_completion())
    finally:
        loop.close() 
